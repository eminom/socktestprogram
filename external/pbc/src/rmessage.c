...
int pbc_get_enum_value(struct pbc_env * env, const char *key, const char *name)
{
    struct _enum * e = _pbcP_get_enum(env, key);
    if (e == NULL) {
		env->lasterror = "Proto enum not found";
		return -1;
	}

    int enum_id = 0;
    int err = _pbcM_si_query(e->name, name , &enum_id);
    if (err) {
        env->lasterror = "pbc_get_enum_value packed invalid enum";
        return -1;
    }
